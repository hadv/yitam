...
export function TailwindApiKeySettings({ onApiKeySet, socket }: TailwindApiKeySettingsProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Notify socket about API key update
      if (socket && socket.connected) {
        socket.emit('update-api-key', apiKey);
      }

      // Call the callback without reloading the page
      onApiKeySet();
    } catch (error) {
