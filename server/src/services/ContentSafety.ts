...
export class ContentSafetyService {
  ...
  public async validateResponse(content: string, language: Language): Promise<void> {
    // Skip validation entirely if AI content safety is disabled and no cached result
    if (!this.config.useAiContentSafety) {
      // Only do a basic prompt injection check for critical patterns
      if (SAFETY_RULES.PROMPT_INJECTION.SYSTEM_PROMPT.patterns.some(pattern => pattern.test(content))) {
        throw new ContentSafetyError(
          SAFETY_RULES.PROMPT_INJECTION.SYSTEM_PROMPT.message[language],
          'prompt_injection',
          language
        );
      }
  ...
  /**
   * Fast basic check for prompt injection only - no AI validation
   * Used when AI content safety is disabled
   */
  public checkPromptInjectionOnly(content: string, language: Language): boolean {
    // Skip empty content
    if (!content || content.trim().length === 0) {
