...
io.use((socket, next) => {
  ...
  // Store the user data and API key in the socket object for later use
  socket.data.user = { email: userEmail, name: userName, apiKey };
  next();
});
...
io.on('connection', (socket: Socket) => {
  ...
  // Initialize Anthropic client for this connection
