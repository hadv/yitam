...
async function readLegalDocument(filename: string) {
  try {
...
export const handleLegalDocumentRequest = async (socket: any, documentType: string) => {
  ...
  const result = await readLegalDocument(filename);
  socket.emit('legal-document', result);
};

// REST API endpoints
router.get('/terms', async (req, res) => {
  const result = await readLegalDocument('terms-and-conditions.md');
  if (result.success) {
    res.json(result);
  } else {
...
router.get('/privacy', async (req, res) => {
  const result = await readLegalDocument('privacy-policy.md');
  if (result.success) {
    res.json(result);
  } else {
    res.status(500).json(result);
